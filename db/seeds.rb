...
emails = ["frantzhed@gmail.com", "yass@gmail.com", "frantzzhed@gmail.com", "yasss@gmail.com", "frantzzzhed@gmail.com", "yassss@gmail.com", "frantzzzzhed@gmail.com", "yasssss@gmail.com"]
description = ["Bonjour je suis Guillaume professeur de ski specialisé dans le ski de fond, je me ferai une joie de vous faire découvrir le domaine afin que vous passiez de très belles vacances !"]
Review.destroy_all
User.destroy_all
...
7.times do |index|
  ...
  Review.create!(content: "Very Good experience", rate: rand(0..5), user_id: User.last.id, booking_id: Booking.last.id)
end

7.times do |index|
  user = User.new(instructor: true, email: Faker::Internet.email , password: "123456", first_name: Faker::Name.first_name, last_name: Faker::Name.last_name, phone_number: Faker::PhoneNumber.cell_phone, description: Faker::Lorem.paragraphs, languages: [User::LANGUAGES.sample, User::LANGUAGES.sample].uniq, ski_resort: "Vars", technical_skill: technical_skill.sample)
  user.photo.attach(io: File.open(Rails.root.join("db/fixtures/user#{index + 1}.jpg")), filename: 'photo.jpg')
  user.save!
  puts "Created one user"
  Category.create!(name: category.sample, note: rand(0..5), user_id: User.last.id )
